...
import (
	...
	"github.com/vvbae/monkey/parser"
)
	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []interface{}{
				[]code.Instructions{
					code.Make(code.OpGetFree, 0),  // get `a` on stack (its a free var)
					code.Make(code.OpGetLocal, 0), // get b from param
					code.Make(code.OpAdd),
					code.Make(code.OpReturnValue),
				},
				[]code.Instructions{
					code.Make(code.OpGetLocal, 0), // get `a` from param
			...
			expectedConstants: []interface{}{
				...
				[]code.Instructions{
					...
					code.Make(code.OpClosure, 0, 2), // fn(c)
					code.Make(code.OpReturnValue),
				},
				[]code.Instructions{
					code.Make(code.OpGetLocal, 0),
